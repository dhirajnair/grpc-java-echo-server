...
public class EchoServer {
  static public void main(String[] args) throws IOException, InterruptedException {
	...
	int port = Integer.parseInt(System.getenv("ECHO_SERVICE_PORT"));
...
class EchoServiceImpl extends EchoServiceGrpc.EchoServiceImplBase {
  ...
  public void echo(EchoRequest request, StreamObserver<EchoResponse> responseObserver) {
    try {
      ...
      int sleep = Integer.parseInt(System.getenv("ECHO_SERVER_SLEEP"));
      System.out.println("Received: " + request.getMessage());
      if(sleep>0) {
    	  System.out.println("Processing["+sleep+"]:" + request.getMessage());
    	  try {
			Thread.sleep(sleep);
